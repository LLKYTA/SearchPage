...
1. **注册** → 调用 \`WidgetFramework.register(type, Class)\`
...
在 \`widgets.js\` 中创建一个继承 \`Widget\` 的类：
...
- **方式二**：页面加载后，点击 \`+\` 按钮打开小组件库，点击“问候语”卡片即可添加
...
| 组件 | type | 说明 |
|------|------|------|
| \`ClockWidget\` | \`clock\` | 实时数字时钟 |
| \`WeatherWidget\` | \`weather\` | 天气显示（UAPI） |
| \`TodoWidget\` | \`todo\` | 待办事项列表 |
| \`BookmarksWidget\` | \`bookmarks\` | 常用书签 |
| \`AiToolsWidget\` | \`ai-tools\` | AI 工具入口 |
| \`ShortcutsWidget\` | \`shortcuts\` | 自定义快捷方式 |
| \`HotboardWidget\` | \`hotboard\` | 多源实时热榜 |
...
开发者可根据 \`widget-sm/md/lg\` 类名在 CSS 中定制不同尺寸的布局细节。
...
如需调整拖拽行为，修改 \`widget-framework.js\` 中 \`_enableDragDrop\` 方法的 \`Sortable\` 配置即可。
...
KD起始页小组件系统提供了接近 iOS 原生小组件体验的 Web 实现，支持拖拽、尺寸切换和持久化，开发者只需关注 \`render()\` 内的业务逻辑，即可快速开发新组件。
